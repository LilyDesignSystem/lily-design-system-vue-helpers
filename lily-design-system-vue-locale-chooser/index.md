# LocaleChooser (Vue helper)

A reusable, headless Vue 3 locale chooser — an icon button that opens
...
```ts
import LocaleChooser from "./lily-design-system-vue-locale-chooser/LocaleChooser.vue";
```
...
Render the chooser with a `label` and the list of locales your app
supports. The control is a globe button; activating it opens a
listbox of the locales. The chooser writes `lang` and `dir` onto
...
import LocaleChooser, {
    localeName,
} from "./lily-design-system-vue-locale-chooser/LocaleChooser.vue";
...
The chooser does NOT translate strings — that is the consumer's i18n
...
- **Element ids come from a module counter** (`nextLocaleChooserId()`),
...
chooser accepts whichever form you prefer in the `locales` array
...
<template>
    <LocaleChooser label="Language" :locales="['en', 'cy']" v-model:value="locale" />
</template>
```
...
By default the chooser uses the English names from `locales.tsv`
...
| `change`        | `string` | After the chooser applies a new locale (consumer-form code). |
...
The compensating pattern is a visible `.locale-chooser-status` region
...
| `CLAUDE.md`                   | `@AGENTS.md`.                                    |
| `LocaleChooser.vue`            | The component implementation.                    |
...
| --------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| [basic.vue](./examples/basic.vue)                                               | The default rendering, plus the `.locale-chooser-status` live region. |
...
| [scoped-target.vue](./examples/scoped-target.vue)                                 | Multiple per-region choosers, each scoped to its own panel.         |
