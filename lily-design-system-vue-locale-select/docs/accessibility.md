...
<div class="locale-select">
    ...
    <ul class="locale-select-list" id="locale-select-1-list" role="listbox"
        ...
        aria-activedescendant="locale-select-1-option-1">
        <li id="locale-select-1-option-0" role="option"
            aria-selected="false" lang="en">English</li>
        <li id="locale-select-1-option-1" role="option"
...
`ThemeSelect`. **This is a language picker, and its own name is
...
`LocaleSelect.test.ts` — but conformance to the pattern is not the
...
<LocaleSelect label="Language" :locales="['en', 'fr', 'ar']" v-model:value="locale">
    <template #default>
        <svg class="locale-select-icon" width="20" height="20"
...
package pairs the select with a status region, and so does the quick
...
kebab-case, consistent with `locale-select`, `locale-select-button`,
and `locale-select-option`.
...
- **The button name never includes the active locale.** There is no
  ...
  `.locale-select-status` live region is what reports the change;
...
both to the same ref: `LocaleSelect` keeps owning the apply lifecycle
