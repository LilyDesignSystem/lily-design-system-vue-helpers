...
.locale-select-list { position: absolute; inset-block-start: 100%; z-index: 50; }
```
...
`.locale-select-list`. `hidden` is only a UA-stylesheet
...
```css
.locale-select-option[data-active] { background: #e5e7eb; }
```
...
Delete the prop; the `.locale-select-placeholder` hook is gone too.
...
`.locale-select-option`, or render your own control alongside and bind
both to one ref. See
...
```css
.locale-select-list { max-block-size: 20rem; overflow-y: auto; }
```

## "The list is cut off by an ancestor"

An ancestor has `overflow: hidden` (or a transform/filter creating a
containing block). The component does not portal. Move the select
...
same ref the select binds to — the recipe in
[recipes.md](./recipes.md#read-a-locale-cookie-before-render-nuxt-3)
does this.
