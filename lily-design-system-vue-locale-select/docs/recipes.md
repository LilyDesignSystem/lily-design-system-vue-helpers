...
This mirrors `detect-from-system` on theme-select.

## Show each language in its own language (endonyms)

The single highest-value thing you can do for a locale select. A user
...
The flicker-free path: resolve on the server, hand the select an
explicit `value`, and let `value`-beats-everything do the rest.

```vue
<script setup lang="ts">
import LocaleSelect, { bcp47LocaleTag, isRtlLocale } from "lily-design-system-vue-locale-select";
...
the select then re-applies the same values on mount, so nothing
...
<template>
    <section ref="panel">
        <LocaleSelect label="Quotation language" :locales="['en', 'ar']" :target="panel" />
...
`change` fires *after* the select has applied `lang` / `dir` /
...
The select owns the selection, not the formatting. Derive your
formatters from the bound ref:

```vue
<script setup lang="ts">
import { computed, ref } from "vue";
import { bcp47LocaleTag } from "lily-design-system-vue-locale-select";
...
<template>
    ...
    <p class="locale-select-status" role="status" :lang="bcp47LocaleTag(locale)">
        {{ localeName(locale) }}
    </p>
</template>
```
...
and bind both to the same ref — the select keeps owning the lifecycle:

```vue
<LocaleSelect label="Language" :locales="ALL" v-model:value="locale" />
...
```vue
<LocaleSelect label="Language / Idioma" :locales="['en', 'es']" />
```
