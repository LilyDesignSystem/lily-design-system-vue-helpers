...
**Package renamed: `lily-design-system-vue-text-size-picker` → `@lilydesignsystem/vue-text-size-picker`.** npm scoped packages
...
relative to `lily-design-system-vue-text-size-picker`'s last published version (`0.2.0`);
...
Renamed from `lily-design-system-vue-text-size-select`, alongside its
three sibling helpers. Nothing was ever published under the old name, so
the version resets to 0.1.0 rather than continuing the in-tree 0.2.0.
