...
| `.theme-select-icon`      | The `<span>` wrapping the default `◑` glyph. **Absent** when you pass a default slot — the slot replaces it. |
...
If you pass a default slot, everything except `.theme-select-icon` is
...
| --------------------------------- | --------------------------- | ---------------------------------------------------- |
| `aria-expanded="true"`            | `.theme-select-button`      | Style the trigger's open state.                      |
| `hidden`                          | `.theme-select-list`        | Present while closed. Prefer `[hidden]` over a class. |
| `aria-selected="true"`            | `.theme-select-option`      | The **committed** selection — the active theme.      |
| `data-active`                     | `.theme-select-option`      | The **keyboard-active** option while the listbox is open. |
...
`.theme-select-list` (for example `display: flex`), you override that
...
Style `.theme-select-list:focus-visible` as well as the button: the
...
- Don't hide the `.theme-select-button` with `display: none`. It is
...
- Don't set `display` on `.theme-select-list` without restoring
...
.my-theme-select :deep(.theme-select-button) {
    /* :deep targets descendants regardless of the scoped boundary */
}
.my-theme-select :deep(.theme-select-option[data-active]) {
    /* … */
}
</style>
```
...
except `.theme-select` itself needs it.
