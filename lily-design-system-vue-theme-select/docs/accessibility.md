...
catalog entry
(`lily-design-system-vue-headless/components/ThemeSelect/`) is still a
...
Every example in this package pairs the select with a status region,
...
`.theme-select-button`, and the `.theme-select-list` itself while it is
...
- The `.theme-select-status` live region should fire once per change
...
- **Forgetting to translate `themeLabels`.** The select only knows what
...
- **Dropping the `.theme-select-status` region to save space.** With an
