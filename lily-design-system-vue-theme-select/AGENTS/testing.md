...
import ThemeSelect, { themeHref, normaliseThemesUrl } from "./ThemeSelect.vue";
...
```ts
const button = wrapper.find("button.theme-select-button");
const list = wrapper.find("ul.theme-select-list");
const options = wrapper.findAll("li.theme-select-option");
```
...
```ts
const wrapper = mount(ThemeSelect, { props: { /* … */ } });
...
await wrapper.findAll("li.theme-select-option")[1].trigger("click");
await flush();
```
...
`nextThemeSelectId()` and differ per mount:
...
custom content is inside the button and that `.theme-select-icon` is
gone:
...
await flush();
expect(wrapper.find("button.theme-select-button").text()).toContain("custom glyph");
