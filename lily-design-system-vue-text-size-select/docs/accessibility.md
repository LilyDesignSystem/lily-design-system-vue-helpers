...
(`lily-design-system-vue-headless/components/TextSizeSelect/`) is still
...
fallback, it stays monochrome like `theme-select`'s `◑`, and it is the
...
Pair the select with a status region. **Shipping it is the default;
...
Use the `.text-size-select-status` class hook for the element.
...
The select does not suppress `:focus` or `:focus-visible` styling. Two
elements take focus and both need a visible indicator: the
`.text-size-select-button`, and the `.text-size-select-list` itself
...
The select performs no animation. If your CSS transitions `font-size`
...
- The `.text-size-select-status` live region should fire once per
...
- **Dropping the `.text-size-select-status` region to save space.**
