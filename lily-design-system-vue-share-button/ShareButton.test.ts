...
function parts(wrapper: VueWrapper<any>) {
    return {
        button: wrapper.find("button.share-button-trigger"),
        list: wrapper.find("ul.share-button-list"),
        status: wrapper.find("p.share-button-status"),
    };
}
...
describe("ShareButton — markup contract (§7.1–§7.6)", () => {
    ...
    test("§7.1 the button renders ↪, hidden from assistive tech", () => {
        const wrapper = build();
        const icon = wrapper.find(".share-button-icon");
    ...
    test("§7.3 destinations are real links, not role=menuitem", async () => {
        ...
        const links = wrapper.findAll("a.share-button-target");
    ...
    test("§7.5 the copy item renders only when copyLabel is supplied", async () => {
        const bare = build();
        await openList(bare);
        expect(bare.find(".share-button-copy").exists()).toBe(false);

        const withCopy = build({ copyLabel: "Copy link" });
        await openList(withCopy);
        const copy = withCopy.find(".share-button-copy");
    ...
    test("§4.2 the root carries the class hook, the consumer class, and $attrs", () => {
        ...
        const root = wrapper.find("div.share-button");
        expect(root.classes()).toContain("share-button");
...
describe("ShareButton — url resolution and custom glyph (§7.20–§7.22)", () => {
    test("§7.20 an explicit url prop wins", async () => {
        const wrapper = build();
        await openList(wrapper);
        const href = wrapper.find(".share-button-target").attributes("href")!;
