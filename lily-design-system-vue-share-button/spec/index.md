...
Single source of truth for the `lily-design-system-vue-share-button`
...
- `ShareButton.vue` — the implementation
...
- **A dismissed native sheet is not a failure.** `navigator.share()`
  ...
  the interaction.
- **This helper is a disclosure, not a listbox.** The three `*-select`
  helpers in this catalog are APG listboxes with
...
matching how `onChange` maps to `@change` on the `*-select` helpers.
...
- The outside-click listener is registered in `onMounted` and removed
  in `onBeforeUnmount`, matching `theme-select`.
...
- Package: lily-design-system-vue-share-button
- Version: 0.1.0
- License: MIT
- Ported from: `lily-design-system-svelte-share-button` 0.1.0
