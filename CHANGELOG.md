...
- **All four helpers renamed to `*-picker`, and all four reset to
  ...
  exported symbols (`ThemePicker` → `ThemePicker`, `nextSharePickerId` →
  `nextSharePickerId`, …), class hooks (`.theme-picker` →
  `.theme-picker` and every derivative), and data attributes
  (`data-lily-theme-select` → `data-lily-theme-picker`, and the locale /
...
- **`share-picker` loses its trigger naming exception.** The trigger
...
- **A fourth helper: `lily-design-system-vue-share-picker` at 0.1.0.**
  ...
  class hook is `share-picker-button`, following the same
...
- **All three helpers** — `theme-picker`, `locale-picker`, and
  `text-size-picker` — are no longer native `<select>`
  ...
  (U+25D1 CIRCLE WITH RIGHT HALF BLACK) for `theme-picker`, 🌐
  (U+1F310 GLOBE WITH MERIDIANS) for `locale-picker`, and `A`
  (U+0041 LATIN CAPITAL LETTER A) for `text-size-picker`, each wrapped
  in `aria-hidden="true"` so the accessible name always comes from
  `aria-label`.

  `theme-picker` and `locale-picker` converted first;
  `text-size-picker` was deliberately held back and has now joined
...
- **The `placeholder` prop is removed** from `theme-picker` and
  `locale-picker`. It existed
  only to pin the closed `<select>` to a short word; there is no
  `<select>` left to pin, so the 0.3.0 placeholder tradeoff is gone
  along with the `{helper}-placeholder` class hook. `text-size-picker`
  never had the prop.
...
- `sizeName(slug)` on `text-size-picker` — exported label resolver that
...
- **No detection prop on `text-size-picker`.** `theme-picker` has
  `detectFromSystem` and `locale-picker` has `detectFromNavigator`, but
...
- Each helper's `docs/accessibility.md` is rewritten around the three
  new tradeoffs (new file for `text-size-picker`): an icon-only control
  ...
  on platform fonts. On that last point `text-size-picker`'s `A` is
  ...
  only a glyph.
- `text-size-picker`'s page keeps its specific WCAG concern, **1.4.4
