...
relative to `lily-design-system-vue-date-time-picker`'s last published version (`0.2.0`);
...
- An opt-in time-zone `<select>` (`.date-time-picker-time-zone`,
  ...
  never a bundled table — after an empty no-zone option. New prop
  `timeZone` (two-way bindable via `v-model:timeZone`, same
  internal-source-of-truth idiom as `value`), `timeZones`,
  `timeZoneLabels`; the zone rides a hidden `{name}-time-zone` input
  and `data-time-zone` on the root. The value contract is unchanged.
- Tests §7.56–§7.61, one per new acceptance clause (73 in the package,
  315 in the catalog, all green).
