...
function parts(wrapper: VueWrapper<any>) {
    return {
        button: wrapper.find("button.theme-chooser-button"),
        list: wrapper.find("ul.theme-chooser-list"),
        options: wrapper.findAll("li.theme-chooser-option"),
    };
}
...
describe("ThemeChooser — markup contract (§4.2, §7.1–§7.5)", () => {
    ...
    test("§7.1 the root is a div carrying the class hook", () => {
        ...
        expect(wrapper.classes()).toContain("theme-chooser");
...
describe("ThemeChooser — dynamic loading (§5, §7.6–§7.11)", () => {
    ...
    test("§7.8 selecting an option updates href, data-theme, and emits change", async () => {
        ...
        await wrapper.findAll("li.theme-chooser-option")[2].trigger("click");
        await flush();
        expect(document.documentElement.dataset.theme).toBe("abyss");
...
describe("ThemeChooser — system-preference detection (detectFromSystem)", () => {
...
describe("ThemeChooser — spread + custom slot (§7.12–§7.13)", () => {
    ...
    test("§7.13 the default slot replaces the button glyph and receives SlotArgs", async () => {
        ...
        expect(wrapper.find("button.theme-chooser-button").text()).toContain(
            "custom glyph",
        );
